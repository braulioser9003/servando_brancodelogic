...
class Css_For_Elementor_Admin {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name . '_cssfe-waypoints.min', plugin_dir_url( __FILE__ ) . 'js/cssfe-waypoints.min.js', array( 'jquery' ), $this->version, false );
		...
		wp_localize_script($this->plugin_name . '_admin', 'ajax_object', array( admin_url('admin-ajax.php')));
	}
	...
	public function admin_nav_menu(){
		...
		*/
		if( get_option( 'front_on' ) == 'front-on' ){
			...
			add_menu_page( 'ElementsCSS' , 'ElementsCSS', $capability, $parent_slug, [$this , 'set_control'], 'dashicons-align-right', 6 );
		...
		//add_submenu_page( $parent_slug, 'Widget List', __( 'Widget List', 'css-for-elementor' ), $capability, 'widget_list', [$this , 'widget_list'] );
		if( class_exists( 'Css_For_Elementor_Pro' ) ) {
			add_submenu_page( $parent_slug, 'Widget List', __( 'Widget List', 'css-for-elementor' ), $capability, 'go_pro_addon', [$this , 'go_pro_addon'] );
		}else{
	...
	// 	require_once plugin_dir_path( dirname( __FILE__ )) . 'admin/partials/widget-list.php';
	// }
