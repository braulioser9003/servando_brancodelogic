...
class REST_API{
    ...
    public function get_taxonomy_hierarchy( $taxonomy, $parent = 0, $depth = 0 ) {
        ...
        foreach ( $terms as $term ){
            ...
            $svg = $this->get_taxonomy_hierarchy( $taxonomy, $term->term_id, $depth );
            $term->children = $svg;
            $children[ urldecode_deep($term->slug) ] = str_repeat(' - ',$depth).$term->name;
            if (!empty($svg)) {
                $children = array_merge($children, $svg);	
            }
        }
        return $children;
    }
    public function ultp_route_taxonomy_data($prams) {
        ...
        foreach ($all_post_type as $post_type_slug => $post_type ) {
            ...
            foreach ($taxonomies as $key => $taxonomy_slug) {
                $data_term[$taxonomy_slug] = $this->get_taxonomy_hierarchy($taxonomy_slug);
            }
